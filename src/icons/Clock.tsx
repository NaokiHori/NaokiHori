...
function Arc({ radius }: { radius: number }): JSX.Element {
  const start: [number, number] = [radius * Math.cos(angles[0]), radius * Math.sin(angles[0])];
  const end: [number, number] = [radius * Math.cos(angles[1]), radius * Math.sin(angles[1])];
  const d = `
...
function ArrowHead({ radius }: { radius: number }): JSX.Element {
  ...
  const start: [number, number] = [radii[0] * Math.cos(angles[1]), radii[0] * Math.sin(angles[1])];
  const end: [number, number] = [radii[1] * Math.cos(angles[1]), radii[1] * Math.sin(angles[1])];
  const points = `
...
function Hand({ handType, size }: { handType: HandType; size: number }): JSX.Element {
  const x1 = 0;
  const y1 = 0;
...
export const Clock: React.NamedExoticComponent<object> = React.memo(function (): JSX.Element {
  const size = 20;
