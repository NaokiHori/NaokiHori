...
function Topic({ topic }: { topic: string }): JSX.Element {
  return (
    <div className={style.borderedBox} style={{ borderColor: themeColors.foreground }}>
      {topic}
    </div>
  );
}
...
export function Card({ repositoryName }: { repositoryName: string }): JSX.Element {
  const { isCardInfoLoaded, cardInfo }: { isCardInfoLoaded: boolean; cardInfo: CardInfo } =
  ...
  const languages: { name: string; size: number; color: string }[] = cardInfo.languages;
  ...
  if (isCardInfoLoaded) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className={style.cardAnchor}>
        <div className={style.card}>
          ...
          <div className={style.topicsAndLanguages}>
            {languages.map(
              (language: { name: string; size: number; color: string }, key: number) => (
