...
func getTitle(domainSize *util.Size, langs *[]util.LangInfo) map[string]interface{} {
	// move downward a bit, above text
	domainSize.Height += 1.5 * util.GetFontHeight(LargeFont)
	// get total bytes
	getTotalBytes := func(langs *[]util.LangInfo) uint32 {
	...
	result["text"] = fmt.Sprintf("Language Stats (%s)", formatBytes(float64(totalBytes)))
...
func getLangData(mypaths *path.MyPaths, domainSize *util.Size, langs *[]util.LangInfo) []map[string]map[string]interface{} {
	...
	var vSum uint32 = 0
	for _, lang := range *langs {
		l := float64(len(lang.Name)) * w
		if labelWidth < l {
			labelWidth = l
		}
		vMax = max(vMax, lang.Size)
		vSum += lang.Size
	}
	...
	for cntr, lang := range *langs {
		...
		share := func(x *float64) map[string]interface{} {
			// just a text
			shareText := fmt.Sprintf("%.2f%%", 100.*float64(lang.Size)/float64(vSum))
		...
		graph := func(x *float64) map[string]interface{} {
			...
			result["width"] = graphWidth * float64(lang.Size) / float64(vMax)
			result["height"] = rectHeight
			result["round"] = 4
			result["fill"] = lang.Color
			result["stroke"] = lang.Color
			return result
		}(&x)
...
func Create(mypaths *path.MyPaths) {
	var langs []util.LangInfo = fetch(mypaths)
	// overall svg size
